...
#root
root = '/home/jjacob2/runs/island_wave/npzd_itw01/'

#load new ini file
newfile = nc4(root+'initial/roms_island_AnaTcline_MidAdr_ini.nc','r+')

#load grid file
grid = nc4(root+'island_spongeNES_grid.nc','r')

#load mid adreatic test NPZD base on Zavaterelli et al 1998
F_npzd = nc4('/home/jjacob2/runs/bio_toy/midAdr_npzd01/output/roms_his.nc')
...
top = 28.08
bot = 14
cff_up = np.array((-0.2844,-20.114,-467.42,-3550.80))
cff_dn = np.array((0.2655,20.675,536.06,4641.4))
knots = np.array((-21.5, -23, -24.5))



z = np.linspace(-100,0,100)[::-1]
tp = np.empty(z.shape[0])
for i in range(z.shape[0]) :
    if (z[i] <= 0 and z[i] > knots[0]) :
        tp[i] = top
   
    elif (z[i] <=knots[0] and z[i] > knots[1]) :
        tp[i] = cff_up[0]*z[i]**3+cff_up[1]*z[i]**2+cff_up[2]*z[i]+cff_up[3]
    
    elif (z[i] <= knots[1] and z[i] > knots[2]) :
        tp[i] = cff_dn[0]*z[i]**3+cff_dn[1]*z[i]**2+cff_dn[2]*z[i]+cff_dn[3]
        
    elif (z[i] <= knots[2] and z[i] >= -100) :
        tp[i] = bot

lintemp = tp
lindepth = z
#constant salt
salt0 = ones_srhorho*38
...
for d0 in range(temp0.shape[0]) :
    for d2 in range(temp0.shape[2]) :
        for d3 in range(temp0.shape[3]):
            temp0[d0, :, d2, d3] = np.interp(depth[:,d2,d3], 
                                             lindepth[::-1], lintemp[::-1])
